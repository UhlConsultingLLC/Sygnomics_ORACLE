...
export const useConditions = () => useQuery({ queryKey: ['conditions'], queryFn: api.fetchConditions });

export const useMetrics = () => useQuery({ queryKey: ['metrics'], queryFn: api.fetchMetrics });
...
export const useMOADistribution = () => useQuery({ queryKey: ['moaDistribution'], queryFn: api.fetchMOADistribution });

export const useMOACategories = () => useQuery({ queryKey: ['moaCategories'], queryFn: api.fetchMOACategories });
...
export const useLookupDrugMOA = () => useMutation({ mutationFn: (drugName: string) => api.lookupDrugMOA(drugName) });
...
export const useFilterOptions = () => useQuery({ queryKey: ['filterOptions'], queryFn: api.fetchFilterOptions });

export const useFilterTrials = () => useMutation({ mutationFn: (spec: FilterSpec) => api.filterTrials(spec) });

export const usePlot = (plotType: string) =>
  useQuery({ queryKey: ['plot', plotType], queryFn: () => api.fetchPlot(plotType), enabled: !!plotType });

export const useSimulation = () => useMutation({ mutationFn: (req: SimulationRequest) => api.runSimulation(req) });

export const useThreshold = () => useMutation({ mutationFn: (req: ThresholdRequest) => api.learnThreshold(req) });

// TCGA Cohort
export const useTCGASummary = () => useQuery({ queryKey: ['tcgaSummary'], queryFn: api.fetchTCGASummary });
...
export const usePatientProfile = (patientId: string) =>
  useQuery({
    ...
    queryFn: () => api.fetchPatientProfile(patientId),
    enabled: !!patientId,
  });

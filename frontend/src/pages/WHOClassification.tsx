...
export default function WHOClassification() {
  ...
  const [codeletionFilter, setCodeletionFilter, resetCodeletionFilter] = usePersistentState<string>(
    'who_codeletion_filter',
    '',
  );
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', fontSize: '0.85rem', marginBottom: '1.5rem' }}>
        Molecular subtype classification for {totalClassified} trials based on WHO 2021 5th Edition criteria. Integrates
        IDH status, 1p/19q codeletion, MGMT methylation, CDKN2A/B, and H3K27M markers.
      </p>

      <InterpretBox id="who-intro" title="How to read this page">
        <p style={{ margin: '0 0 0.5rem' }}>
          The WHO 5th Edition (2021) overhauled CNS tumor classification by making molecular markers part of the
          diagnosis itself — not just add-ons. This page parses each trial's eligibility text to detect molecular
          requirements and assigns the trial to one or more WHO 2021 subtypes.
        </p>
        <ul style={{ margin: '0.25rem 0 0.5rem 1.1rem', padding: 0 }}>
          <li>
            <strong>Subtype cards (top)</strong> — click to filter the trial table below to only trials assigned that
            WHO type. Trials may target multiple subtypes, so percentages need not sum to 100%.
          </li>
          <li>
            <strong>Classification Confidence</strong> — <span style={{ color: '#2e7d32' }}>High</span> requires an
            explicit WHO subtype or IDH + ≥2 markers; <span style={{ color: '#f57f17' }}>Medium</span> needs IDH or one
            clear marker; <span style={{ color: '#9e9e9e' }}>Low</span> is histological terms only. Trust <em>High</em>{' '}
            for simulation/TAM analyses; <em>Low</em> entries may be mis-classified and should be audited.
          </li>
          <li>
            <strong>IDH Status Requirement</strong> — "IDH excluded" (wildtype) defines the GBM population; "IDH
            required" captures lower-grade astrocytomas and oligodendrogliomas. The remaining buckets catch ambiguous or
            unspecified trials.
          </li>
          <li>
            <strong>Marker badges</strong> (IDH / 1p19q / MGMT columns) — Required, Excluded, Any, Mentioned, Unknown.
            "Mentioned" means the marker is referenced but no direction can be inferred from the eligibility text.
          </li>
          <li>
            <strong>Region</strong> — US (CT.gov) vs EU (CTIS) so you can compare molecular subtyping practices across
            registries.
          </li>
        </ul>
        <p style={{ margin: '0.4rem 0 0', fontSize: '0.78rem', color: '#555' }}>
          <strong>Caveat:</strong> NLP extraction is imperfect. Trials without explicit molecular language default to
          histological assignment or Unknown — always spot-check Low-confidence trials before including them in
          downstream analyses.
        </p>
      </InterpretBox>
      ...
      {stats && (
        <>
          ...
          >
            <h3 style={{ margin: '0 0 1rem', fontSize: '1rem', color: '#333' }}>WHO 2021 Subtype Distribution</h3>
              ...
              {Object.entries(stats.type_distribution).map(([type, count]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ fontSize: '0.7rem', opacity: 0.7, marginTop: 2 }}>{pct}% of trials</div>
                  </button>
                );
              })}
            </div>
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: '1.5rem' }}>
            ...
            >
              <h3 style={{ margin: '0 0 0.8rem', fontSize: '0.95rem', color: '#333' }}>Classification Confidence</h3>
              {['high', 'medium', 'low'].map((level) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <span style={{ fontSize: '0.85rem', textTransform: 'capitalize', width: 65 }}>{level}</span>
                    <div
                      style={{
                        flex: 1,
                        height: 16,
              ...
              >
                <strong style={{ color: '#333' }}>How confidence is assigned:</strong> based on the molecular evidence
                parsed from each trial's eligibility text. <strong style={{ color: '#2e7d32' }}>High</strong> — an
                explicit WHO 2021 subtype is named (e.g. "Glioblastoma, IDH-wildtype") <em>or</em> IDH status is stated
                plus ≥2 other molecular markers. <strong style={{ color: '#f57f17' }}>Medium</strong> — IDH status is
                stated on its own,
                <em> or</em> at least one molecular marker / ≥2 biomarkers are present, <em>or</em> the trial targets a
                single subtype. <strong style={{ color: '#9e9e9e' }}>Low</strong> — minimal or no molecular evidence;
            ...
            >
              <h3 style={{ margin: '0 0 0.8rem', fontSize: '0.95rem', color: '#333' }}>IDH Status Requirement</h3>
              {['excluded', 'required', 'any', 'mentioned', 'unknown'].map((status) => {
                ...
                const label =
                  status === 'excluded'
                    ? 'IDH-wildtype (mutation excluded)'
                    : status === 'required'
                      ? 'IDH-mutant (mutation required)'
                      : status === 'any'
                        ? 'Any IDH status'
                        : status === 'mentioned'
                          ? 'Mentioned (no direction)'
                          : 'Not specified';
                return (
                  <div
                    key={status}
      ...
      >
        {(() => {
          const anyFilter =
            whoTypeFilter || idhFilter || confFilter || codeletionFilter || mgmtFilter || regionFilter || nctSearch;
          ...
          return (
            <>
              ...
              >
                <h3 style={{ margin: 0, fontSize: '1rem', color: '#333' }}>Trials {anyFilter ? '(filtered)' : ''}</h3>
                <button
                  onClick={handleReset}
                  style={{
                    padding: '4px 12px',
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: '0.8rem', alignItems: 'center' }}>
                ...
                <select
                  value={whoTypeFilter}
                  onChange={(e) => {
                    setWhoTypeFilter(e.target.value);
                    setPage(0);
                  }}
                  style={selStyle}
                >
                ...
                <select
                  value={codeletionFilter}
                  onChange={(e) => {
                    setCodeletionFilter(e.target.value);
                    setPage(0);
                  }}
                  style={selStyle}
                >
                  ...
                  <tr style={{ background: '#f8f9fa', borderBottom: '2px solid #dee2e6' }}>
                    ...
                    <th style={{ padding: '8px 10px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                        ...
                        <InlineHelp
                          size={11}
                          text="US = trial sourced from ClinicalTrials.gov; EU = trial sourced from CTIS."
                        />
                      </span>
                    </th>
                    <th style={{ padding: '8px 10px', textAlign: 'left' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                        ...
                        <InlineHelp
                          size={11}
                          text="WHO 2021 CNS 5th edition subtype(s) inferred from eligibility text. A trial may be assigned to multiple subtypes."
                        />
                      </span>
                    </th>
                    <th style={{ padding: '8px 10px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                        ...
                        <InlineHelp
                          size={11}
                          text="WHO grade range required for eligibility (1–4). A single value means a specific grade; a range means any within the window."
                        />
                      </span>
                    </th>
                    <th style={{ padding: '8px 10px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                        ...
                        <InlineHelp
                          size={11}
                          text="IDH1/2 mutation status requirement. Required = mutant-only trial; Excluded = wildtype-only (typical of GBM); Any = both accepted; Mentioned = referenced without direction."
                        />
                      </span>
                    </th>
                    <th style={{ padding: '8px 10px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                        ...
                        <InlineHelp
                          size={11}
                          text="1p/19q codeletion status (defines oligodendroglioma when co-occurring with IDH mutation)."
                        />
                      </span>
                    </th>
                    <th style={{ padding: '8px 10px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                        ...
                        <InlineHelp
                          size={11}
                          text="MGMT promoter methylation status (predictive for temozolomide response in GBM)."
                        />
                      </span>
                    </th>
                    <th style={{ padding: '8px 10px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                        ...
                        <InlineHelp
                          size={11}
                          text="Classification confidence: green = High, orange = Medium, gray = Low. Based on quantity and specificity of molecular evidence in the eligibility text."
                        />
                      </span>
                    </th>
                    <th style={{ padding: '8px 10px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                        ...
                        <InlineHelp
                          size={11}
                          text="Total count of distinct molecular markers referenced in the trial's eligibility text. More markers generally yield higher confidence."
                        />
                      </span>
                    </th>
                  </tr>
                ...
                <tbody>
                  {trials.trials.map((t) => (
                    <tr key={t.nct_id} style={{ borderBottom: '1px solid #eee' }}>
                      ...
                      <td style={{ padding: '6px 10px', textAlign: 'center', color: '#888' }}>{t.biomarker_count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

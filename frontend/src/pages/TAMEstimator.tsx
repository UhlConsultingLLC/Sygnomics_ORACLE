...
import { runTAMEstimate, fetchMOACategoriesForSim, type TAMResponse } from '../services/api';
...
export default function TAMEstimator() {
  ...
  const toggleMOA = (moa: string) => {
    setSelected((s) => (s.includes(moa) ? s.filter((x) => x !== moa) : [...s, moa]));
  };
  ...
  const filteredOptions = options.filter((o) => o.category.toLowerCase().includes(filter.toLowerCase()));
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: 1100 }}>
      <h1 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>TAM Estimator — Predicted Responders by Drug MOA</h1>
      <p style={{ color: '#666', fontSize: '0.85rem', marginBottom: '1rem' }}>
        Estimate the number of GBM patients that would be predicted to respond to each drug MOA class based on the
        learned DCNA threshold and gene expression &gt; 0 within the TCGA cohort. Enter US and worldwide GBM patient
        counts, select drug MOAs that have completed simulations, and the tool multiplies the TCGA response rate against
        your population counts. Unique responders across all MOAs are estimated via the union of responder IDs.
      </p>

      <InterpretBox id="tam-intro" title="How to read this page">
        <p style={{ margin: '0 0 0.5rem' }}>
          This page projects how many GBM patients would be predicted responders, by MOA class, using the learned DCNA
          threshold from simulation runs. The math is intentionally simple:{' '}
          <em>TCGA response rate × annual patient population</em>. The value is in comparing MOA classes and estimating
          total addressable market (TAM) when biomarker-gated.
        </p>
        <ul style={{ margin: '0.25rem 0 0.5rem 1.1rem', padding: 0 }}>
          <li>
            <strong>Response Rate (TCGA)</strong> — fraction of the TCGA cohort flagged as a predicted responder by the
            MOA's simulation (learned DCNA threshold + expression &gt; 0). This is the biomarker-gated prevalence, not
            an observed clinical response rate.
          </li>
          <li>
            <strong>Top-N vs All trials</strong> — <em>Top-N</em> selects the N best-performing drugs per MOA by
            response rate (less noisy; highlights leads).
            <em> All trials aggregated</em> uses every testing-set trial with a majority- or any-vote rule across drugs
            in the class (broader, noisier estimate).
          </li>
          <li>
            <strong>Per-MOA rows (colored bands)</strong> — US and WW projections are on separate rows so you can read
            the absolute patient counts directly. Response rate is repeated for quick comparison across MOAs.
          </li>
          <li>
            <strong>Union total (gold band)</strong> — each TCGA patient is counted <em>at most once</em> across all
            selected MOAs. The union response rate is therefore ≤ the sum of individual MOA rates (overlap is
            discarded), and represents unique patients addressable by any drug in the selected set.
          </li>
          <li>
            <strong>Missing simulations</strong> — MOAs without a completed simulation are excluded and listed in a
            yellow warning banner above the table.
          </li>
        </ul>
        <p style={{ margin: '0.4rem 0 0', fontSize: '0.78rem', color: '#555' }}>
          <strong>Caveat:</strong> TAM is an upper-bound market estimate assuming perfect biomarker deployment and
          uniform drug access. It does not model adherence, pricing, or clinical response beyond what the DCNA threshold
          predicts.
        </p>
      </InterpretBox>
          ...
          {filteredOptions.map((o) => (
            <label
              ...
              style={{
                ...
                padding: '2px 4px',
                cursor: 'pointer',
                fontWeight: o.is_group ? 600 : 400,
              }}
            >
              <input type="checkbox" checked={selected.includes(o.value)} onChange={() => toggleMOA(o.value)} />
      ...
      {result && (
        <div>
          {result.missing_moas.length > 0 && (
            ...
            >
              <strong>Missing simulations:</strong> {result.missing_moas.join(', ')} — run a simulation for these MOAs
              first.
            </div>
          )}
          <div style={{ fontSize: '0.8rem', color: '#666', marginBottom: '0.5rem' }}>
            TCGA cohort size: {fmt(result.cohort_total)} ·{' '}
            {result.top_n > 0 ? `Top-${result.top_n} drugs per MOA` : `All trials aggregated (${result.rule} rule)`}
          </div>
                ...
                {result.per_moa.map((r) =>
                  r.top_drugs && r.top_drugs.length ? (
                    <li key={r.moa_category}>
                      ...
                      {r.top_drugs.map((d) => `${d.drug_name} (${pct(d.response_rate)})`).join(', ')}
                    </li>
                  ) : null,
                )}
              </ul>
          ...
          <div style={{ marginTop: '0.75rem', fontSize: '0.75rem', color: '#888' }}>
            Response rates computed against the {result.cohort_total}-patient TCGA cohort using each MOA's most recent
            simulation (learned DCNA threshold + expression &gt; 0, aggregated across testing trials with the{' '}
            {result.rule} rule). The union row counts each TCGA patient at most once across all selected MOAs.
          </div>
        </div>
      )}
    </div>

...
function buildPanelTraces(
  ...
  threshold: number,
  exprThresh: number,
  sharedRange?: { xMin: number; xMax: number; yMin: number; yMax: number },
  ...
  if (sharedRange) {
    xMin = sharedRange.xMin;
    xMax = sharedRange.xMax;
...
export default function TrialEligibilityComparison() {
  ...
  const handleSnapshot = async () => {
    ...
    try {
      ...
      const rect = (x: number, y: number, w: number, h: number, fill: string, stroke = '#d7dce3', rx = 6) =>
      ...
      const fields: Field[] = [
        ...
        {
          label: 'Drug targets:',
          value: result.drug_targets.join(', ') || '(none in expression data)',
          col: 3,
          row: 0,
        },
        ...
        { label: 'Expression threshold:', value: result.expression_threshold.toFixed(2), col: 1, row: 1 },
        {
          ...
          value: `${(result.left_panel.stats.response_rate * 100).toFixed(1)}%`,
          col: 2,
          row: 1,
          colSpan: 2,
        },
      ...
      out += text(infoX0, critY, `TCGA-matchable criteria applied (${incl.length + excl.length})`, {
        size: 13,
        weight: 700,
        fill: '#1c3e72',
      });
      ...
      if (result.unmapped_biomarkers.length > 0) {
        out += text(infoX0, critY2 + 36, `Ignored (unmappable): ${result.unmapped_biomarkers.length}`, {
          size: 12,
          fill: '#888',
        });
      }
      ...
      out += text(diffCX, rowY + 28, 'SATGBM vs Trial', {
        size: 11,
        weight: 600,
        fill: '#60656e',
        anchor: 'middle',
      ...
      const pctMissed =
        result.right_panel.stats.responders > 0
          ...
          : 0;
      out += drawBigMetric(m1y, `${pctMissed.toFixed(1)}%`, '#1b7a3a', 'percentage of predicted responders recovered');
      ...
      const drawTile = (
        ...
        label: string,
        value: string,
        dots?: { fill: string; border: string; bw: number }[],
        plusBetween?: boolean,
      ) => {
        ...
        lines.forEach((ln, li) => {
          s += text(tx + w / 2, labelStartY + li * labelLineH, ln, {
            size: labelFontSize,
            weight: 600,
            fill: '#60656e',
            anchor: 'middle',
      ...
      out += drawTile(
        ...
        w1Eligible,
        'Trial Eligible',
        String(result.left_panel.stats.enrolled ?? 0),
        [orangeElig, grayElig],
        true,
      );
      out += drawTile(
        ...
        row1Y,
        w1Responders,
        'Trial Eligible Predicted Responders',
        String(result.left_panel.stats.responders),
        [orangeElig],
      );
      // Response rate tile: fraction layout (numerator / denominator with dividing line)
      {
        ...
        const rrLines: string[] = ['Percentage of TCGA patients predicted to', 'respond based on trial criteria'];
        const rrLineH = 11;
        const rrLabelStartY = row1Y + 10 + rrLineH;
        rrLines.forEach((ln, li) => {
          s += text(tx + w / 2, rrLabelStartY + li * rrLineH, ln, {
            size: 8,
            weight: 600,
            fill: '#60656e',
            anchor: 'middle',
      ...
      out += drawTile(
        ...
        row1Y,
        w1ObservedRR,
        'Observed Clinical Response Rate',
        result.observed_response_rate != null ? `${(result.observed_response_rate * 100).toFixed(1)}%` : 'N/A',
      );
      ...
      out += drawTile(
        ...
        row2Y,
        row2TileW,
        'SATGBM Predicted Responders',
        String(result.right_panel.stats.responders ?? 0),
        [orangeInelig, orangeElig],
        true,
      );
      out += drawTile(
        tilesX + (row2TileW + tileGap),
        row2Y,
        row2TileW,
        'Predicted Responders Missed by Trial Criteria',
        String(result.left_panel.stats.responders_missed),
        [orangeInelig],
      );
      out += drawTile(
        tilesX + (row2TileW + tileGap) * 2,
        row2Y,
        row2TileW,
        'Predicted Non-responders Spared',
        String(result.diff.nonresponders_spared),
        [grayElig],
      );
  ...
  useEffect(() => {
    ...
    const exportConfig: any = {
      ...
      toImageButtonOptions: {
        format: 'svg' as const,
        filename: provenanceImageFilename(`${result.nct_id}_comparison`),
        width: 800,
        height: 680,
        scale: 2,
      },
    ...
    if (plotRef.current) {
      ...
      Plotly.newPlot(
        plotRef.current,
        traces,
        withProvenance(layout, `/trial-comparison/${result.nct_id}`),
        exportConfig,
      );
    }
  }, [result]);
  ...
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      ...
      <p style={{ color: '#555', marginTop: 0 }}>
        Compares how a single clinical trial's enrollment would look on the TCGA-GBM cohort under the trial's stated
        eligibility criteria vs SATGBM's learned TNAS + expression thresholds.
      </p>

      <InterpretBox id="trial-comparison-intro" title="How to read this page">
        <p style={{ margin: '0 0 0.5rem' }}>
          For one trial and one of its drugs, we project who the trial would <em>actually</em> enroll (using its written
          eligibility criteria) vs. who SATGBM's learned biomarker thresholds would enroll — both applied to the same
          TCGA-GBM cohort. The scatter plot colors each patient by predicted responder status; borders indicate
          eligibility under the trial's criteria.
        </p>
        <ul style={{ margin: '0 0 0.4rem 1.1rem', padding: 0 }}>
          <li>
            <strong>Orange dot</strong> — predicted <em>responder</em> (DCNA + expression above thresholds).
          </li>
          <li>
            <strong>Grey dot</strong> — predicted <em>non-responder</em>.
          </li>
          <li>
            <strong>Thick black border</strong> — eligible per the trial's written criteria.
          </li>
          <li>
            <strong>No border</strong> — excluded by the trial's criteria.
          </li>
        </ul>
        ...
        <ul style={{ margin: '0 0 0.3rem 1.1rem', padding: 0 }}>
          <li>
            <strong>Responders recovered</strong> — predicted responders that the trial's criteria would exclude but
            SATGBM's biomarker would enroll. Higher = more potential patients the trial's eligibility language is
            missing.
          </li>
          <li>
            <strong>Non-responders spared</strong> — patients the trial would enroll even though SATGBM predicts they
            wouldn't respond. Higher = more recruitment waste the biomarker could avoid.
          </li>
          <li>
            <strong>Observed Clinical Response Rate</strong> — what this trial actually reported. If SATGBM's predicted
            rate is close to this, the biomarker is well-calibrated for this drug.
          </li>
        </ul>
        <p style={{ margin: 0, color: '#555', fontSize: '0.8rem' }}>
          Use the arm / sub-population selector to scope the comparison to a specific cohort within the trial (e.g. a
          biomarker-positive sub-group reported in the outcome tables).
        </p>
      </InterpretBox>
      ...
      <div style={{ ...panelStyle, display: 'flex', alignItems: 'center', gap: '0.8rem', marginBottom: '1rem' }}>
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') handleLoadDrugs();
          }}
        />
      ...
      {drugOptions && (
        <div style={{ ...panelStyle, marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                ...
                <label
                  style={{
                    ...
                    borderRadius: 4,
                    border:
                      selectedArmId === null && selectedSubgroup === null ? '1px solid #1c3e72' : '1px solid #dee3ea',
                ...
                >
                  ...
                  />
                  <div>
                    <div style={{ fontWeight: 600, color: '#1c3e72', fontSize: '0.88rem' }}>
                      Full trial (all arms &amp; groups)
                    </div>
                  </div>
                </label>
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            checked={isSelected}
                            onChange={() => {
                              setSelectedSubgroup(sg.group_title);
                              setSelectedArmId(null);
                            }}
                            style={{ marginTop: 3 }}
                          />
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            {sg.group_description && (
                              <div style={{ fontSize: '0.76rem', color: '#555', marginTop: 2, lineHeight: 1.35 }}>
                                {sg.group_description.length > 250
                                  ? sg.group_description.slice(0, 250) + '…'
                                  : sg.group_description}
                              </div>
                            )}
                          </div>
          ...
          <div style={{ fontWeight: 600, color: '#1c3e72', marginBottom: '0.6rem' }}>Select a drug to analyze</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', marginTop: '0.9rem' }}>
            <button style={buttonStyle} onClick={handleRun} disabled={loading || selectedDrugId == null}>
      ...
      {result && (
        <>
          <div ref={captureRef}>
            <div style={{ ...panelStyle, marginBottom: '1rem' }}>
              ...
              >
                ...
                <div>
                  <strong style={{ marginRight: 6 }}>Trial:</strong>
                  {result.nct_id}
                </div>
                <div>
                  <strong style={{ marginRight: 6 }}>Drug:</strong>
                  {result.drug}
                </div>
                <div>
                  <strong style={{ marginRight: 6 }}>MOA:</strong>
                  {result.moa_category.replace(/^group:/, '')}
                </div>
                <div>
                  <strong style={{ marginRight: 6 }}>Targets:</strong>
                  {result.drug_targets.join(', ') || '(none)'}
                </div>
                {/* Row 2: model params + headline prediction */}
                <div>
                  <strong style={{ marginRight: 6 }}>Learned TNAS threshold:</strong>
                  {result.learned_dcna_threshold.toFixed(4)}
                </div>
                <div>
                  <strong style={{ marginRight: 6 }}>Expression threshold:</strong>
                  {result.expression_threshold.toFixed(2)}
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', minWidth: 0 }}>
                      ...
                      <>
                        ...
                        <div style={{ fontSize: '0.85rem', textAlign: 'left' }}>
                          ...
                          {result.unmapped_biomarkers.length > 0 && (
                            <div style={{ color: '#888' }}>
                              <span style={{ fontWeight: 600 }}>Ignored (unmappable):</span>{' '}
                              {result.unmapped_biomarkers.length}
                            </div>
                          )}
                        </div>
                      </>
                ...
                <div style={{ ...panelStyle, textAlign: 'center' }}>
                  ...
                  >
                    ...
                    <InlineHelp
                      size={11}
                      text="Of all the patients SATGBM predicts would respond, what fraction does the biomarker recover that the trial's criteria would have missed. Higher is better."
                    />
                  </div>
                  ...
                  <div
                    style={{
                      fontSize: '1.7rem',
                      fontWeight: 700,
                      color: result.diff.responders_recovered >= 0 ? '#1b7a3a' : '#b00020',
                    }}
                  >
                  ...
                  >
                    ...
                    <InlineHelp
                      size={11}
                      text="Absolute count of patients SATGBM would enroll but the trial's criteria would exclude, and who SATGBM predicts would respond. Green = biomarker helps find more responders."
                    />
                  </div>
                  <hr style={{ border: 'none', borderTop: '1px solid #e0e4ea', margin: '0.9rem 0' }} />
                  <div
                    style={{
                      fontSize: '1.7rem',
                      fontWeight: 700,
                      color: result.diff.nonresponders_spared >= 0 ? '#1b7a3a' : '#b00020',
                    }}
                  >
                  ...
                  >
                    ...
                    <InlineHelp
                      size={11}
                      text="Patients the trial would have enrolled but SATGBM predicts wouldn't respond. Screening these out reduces enrollment cost without losing responders."
                    />
                  </div>
                </div>
                        ...
                        const effectColor =
                          a.response_effect === 'increased_response' || a.response_effect === 'sensitivity'
                            ...
                            : a.response_effect === 'resistance' || a.response_effect === 'decreased_response'
                              ...
                              : '#60656e';
                        const effectLabel =
                          a.response_effect === 'increased_response'
                            ? '▲ Increased response'
                            : a.response_effect === 'sensitivity'
                              ? '▲ Sensitivity'
                              : a.response_effect === 'resistance'
                                ? '▼ Resistance'
                                : a.response_effect === 'decreased_response'
                                  ? '▼ Decreased response'
                                  : a.response_effect === 'no_effect'
                                    ? '— No effect'
                                    : a.response_effect.replace(/_/g, ' ');
                        const levelLabel =
                          a.evidence_level === 'level_1'
                            ? 'L1'
                            : a.evidence_level === 'level_2'
                              ? 'L2'
                              : a.evidence_level === 'level_3'
                                ? 'L3'
                                : a.evidence_level === 'level_4'
                                  ? 'L4'
                                  : '?';
                        const levelColor =
                          a.evidence_level === 'level_1'
              ...
              <div style={panelStyle}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.8rem' }}>
                  ...
                  <StatTile
                    ...
                    value={
                      result.observed_response_rate != null
                        ? `${(result.observed_response_rate * 100).toFixed(1)}%`
                        : 'N/A'
                    }
          ...
          <div style={{ marginTop: '0.8rem' }}>
            <button style={{ ...buttonStyle, background: '#1c3e72' }} onClick={handleSnapshot}>
              📸 Save snapshot (SVG)
            </button>
          </div>

          <div style={{ ...panelStyle, marginTop: '1rem' }}>
            <button style={{ ...buttonStyle, background: '#4a6a99' }} onClick={() => setShowBiomarkers((s) => !s)}>
            ...
            {showBiomarkers && (
              <div style={{ marginTop: '0.8rem', fontSize: '0.85rem' }}>
                <div style={{ marginBottom: 8 }}>
                  ...
                  <ul>
                    {result.extracted_biomarkers
                      ...
                      .map((b, i) => (
                        <li key={i}>
                          {b.marker} <em style={{ color: '#60656e' }}>({b.context})</em>
                        </li>
                      ))}
                ...
                <div>
                  ...
                  <ul>
                    {result.unmapped_biomarkers.map((b, i) => (
                      <li key={i}>{b}</li>
                    ))}

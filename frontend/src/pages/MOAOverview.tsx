...
export default function MOAOverview() {
  ...
  const [selectedRowKey, setSelectedRowKey, resetSelectedRowKey] = usePersistentState<string>(
    'moa_selected_row_key',
    '',
  );
  const [filters, setFilters, resetFilters] = usePersistentState<{
    broad: string;
    moa: string;
    interventions: string;
    trials: string;
  }>('moa_filters', MOA_FILTERS_INITIAL);
    ...
    <div>
      ...
      <InterpretBox id="moa-overview-intro" title="How to read this page">
        <p style={{ margin: '0 0 0.5rem' }}>
          Each trial intervention (drug, combination, device) is mapped to a mechanism-of-action (MOA) via Open Targets
          / ChEMBL. MOAs are grouped hierarchically: a <em>long-form</em> description (e.g., "Epidermal growth factor
          receptor (EGFR) inhibitor") is condensed to a <em>short form</em> ("EGFR inhibitor") and rolled up into a{' '}
          <em>broad category</em> ("Kinase inhibitor"). Grouping lets downstream pages (simulation, TAM, screening)
          aggregate across similar drugs.
        </p>
        <ul style={{ margin: '0.25rem 0 0.5rem 1.1rem', padding: 0 }}>
          <li>
            <strong>Drug MOA Lookup</strong> — queries Open Targets live for any drug name. Useful for spot-checking a
            single intervention without running a full re-classification.
          </li>
          <li>
            <strong>Classify New Interventions</strong> — processes only interventions without existing MOA annotations.{' '}
            <strong>Re-classify All</strong> wipes and rebuilds annotations (use after ontology updates).
          </li>
          <li>
            <strong>Distribution chart</strong> — trial count per MOA category. Tall bars indicate well-studied
            mechanisms in the corpus; thin tail reveals rare/novel MOAs where data is sparse.
          </li>
          <li>
            <strong>Categories table</strong> — filterable by broad category, MOA, minimum intervention count, or
            minimum trial count. Click <em>View</em> on any row to inspect the underlying interventions, their long-form
            descriptions, target genes, and data source (Open Targets vs fallback).
          </li>
        </ul>
        <p style={{ margin: '0.4rem 0 0', fontSize: '0.78rem', color: '#555' }}>
          <strong>Caveat:</strong> Non-drug interventions (radiation, surgery, device) are tagged <code>NON_DRUG</code>{' '}
          with a subtype from the CT.gov
          <code>intervention_type</code> field; they are excluded from most downstream biomarker analyses.
        </p>
      </InterpretBox>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 4 }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid #ddd' }}>
                    <th style={{ textAlign: 'left', padding: '4px 8px', fontSize: '0.8rem', color: '#666' }}>
                      Long Form
                    </th>
                    <th style={{ textAlign: 'left', padding: '4px 8px', fontSize: '0.8rem', color: '#666' }}>
                      Short Form
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: '1rem' }}>
        ...
        {classifyMutation.data && (
          <span style={{ fontSize: '0.85rem', color: '#666', alignSelf: 'center' }}>
            Classified: {classifyMutation.data.classified}, Skipped: {classifyMutation.data.skipped}, Failed:{' '}
            {classifyMutation.data.failed}
          </span>
        )}
        <button
          onClick={handleReset}
                  ...
                  <thead>
                    <tr>
                      ...
                      <th style={{ textAlign: 'right', padding: '0.5rem', borderBottom: '2px solid #ddd' }}>
                        ...
                        >
                          Interventions
                          <InlineHelp size={11} text="Distinct drug/combination records mapped to this category." />
                        </span>
                      </th>
                      <th style={{ textAlign: 'right', padding: '0.5rem', borderBottom: '2px solid #ddd' }}>
                        <span
                      ...
                      <th style={{ textAlign: 'center', padding: '0.5rem', borderBottom: '2px solid #ddd' }}>
                        Details
                      </th>
                    </tr>
                    <tr>
                      ...
                      return (
                        <tr key={rowKey} style={{ borderBottom: '1px solid #eee' }}>
                          ...
                          <td style={{ padding: '0.4rem 0.5rem', textAlign: 'center' }}>
                            <button
                              onClick={() => setSelectedRowKey(selectedRowKey === rowKey ? '' : rowKey)}
                              style={{
                                padding: '2px 10px',
                                fontSize: '0.78rem',
                                background: 'transparent',
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.83rem' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ textAlign: 'left', padding: '0.4rem', borderBottom: '2px solid #ddd' }}>
                    MOA (Long Form)
                  </th>
              ...
              <tbody>
                {interventions.map((iv, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                    ...
                    <td
                      title={iv.mechanism_description || ''}
                      style={{
